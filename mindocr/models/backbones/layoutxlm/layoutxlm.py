...
from mindocr.models.backbones.layoutxlm.configuration import LayoutXLMPretrainedConfig
from mindocr.models.backbones.layoutxlm.visual_backbone import build_resnet_fpn_backbone, read_config
from mindocr.models.backbones._registry import register_backbone
...
def relative_position_bucket(relative_position, bidirectional=True, num_buckets=32, max_distance=128):
    ...
    val_if_large = max_exact + (
        ops.log(n.astype(ms.float32) / max_exact) / math.log(max_distance / max_exact) * (
...
class LayoutXLMSelfAttention(nn.Cell):
    def __init__(self, config):
        ...
        if config.fast_qkv:
            self.qkv_linear = nn.Dense(config.hidden_size, 3 * self.all_head_size,
                                       has_bias=False).to_float(self.dense_dtype)
    ...
    def __init__(self, config):
        ...
        if self.use_visual_backbone is True:
            ...
            self.visual_proj = nn.Dense(
                config.image_feature_pool_shape[-1], config.hidden_size).to_float(self.dense_dtype)
...
class LayoutXLMForTokenClassification(nn.Cell):
    def __init__(self, layoutxlm, num_classes=2,use_float16=False, dropout=None):
        ...
        self.classifier = nn.Dense(self.layoutxlm.config.hidden_size, num_classes).to_float(self.dense_dtype)
...
class REDecoder(nn.Cell):
    ...
    def build_relation(self, relations, entities):
        ...
        for b in range(batch_size):
            ...
            relation_per_doc_label = ops.zeros((ops.shape(reordered_relations)[0], 1),
                                               dtype=reordered_relations.dtype)
            ...
            new_relations[b, 1: ops.shape(relation_per_doc)[0] + 1] = relation_per_doc
    ...
    def construct(self, hidden_states, entities, relations):
        ...
        for b in range(batch_size):
            ...
            #     print("shape---", i.shape)
            # print("====================")
            pred_relations_len = len(pred_relations)
            if pred_relations_len > 0:
                ...
                all_pred_relations[b, 0, :, :] = ms.tensor(pred_relations_len, dtype=all_pred_relations.dtype)
                for i in range(0, pred_relations_len):
                    all_pred_relations[b, i + 1, :, :] = pred_relations[i]
...
class NLPBaseModel(nn.Cell):
    def __init__(self,
        ...
        pretrained_model_dict = {
            LayoutXLMModel: {
                "base": "https://download.mindspore.cn/toolkits/mindocr/kie/layoutxlm-base-uncased.ckpt",  # TODO:download link
                "vi": "https://download.mindspore.cn/toolkits/mindocr/kie/vi-layoutxlm-base-uncased.ckpt"
            }
        }
        if checkpoints is not None:  # load the trained model
            config = LayoutXLMPretrainedConfig(mode, use_float16)
            base_model = base_model_class(config)
            if type == "ser":
                model = model_class(
                    base_model, num_classes=kwargs["num_classes"], dropout=None)
            else:
                model = model_class(base_model, dropout=None)
            ...
            if pretrained is True:
                load_pretrained(base_model, pretrained_model_link)
            if type == "ser":
...
class layoutxlm_for_ser(NLPBaseModel):
    def __init__(self,
        ...
        super(layoutxlm_for_ser, self).__init__(
            LayoutXLMModel,  # base_model_class
            LayoutXLMForTokenClassification,  # model_class
            mode,
            "ser",
            pretrained,
            checkpoints,
